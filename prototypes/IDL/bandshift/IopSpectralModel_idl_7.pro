...
;   Only these wavelengths (or floating point wavelengths, that correspond to these wavelengths after truncation (fix)) are allowed.  
;
; :Requires:
;   IDL 8.0
;
; :Todo:
;   Allow the routine to store A and B Bricaud coefficients for non-integer wavelengths.
;   
; :Params:
; 
...
;     
;     If the wavelengths are given as float, the A and B coefficients of the truncated float (fix) are returned. 
;     
...
PRO get_A_B_Bricaud, wl, a_bricaud, b_bricaud
 ...
 ;  b_bricaud = fltarr(n_wl)
 ;  for i=0,n_wl-1 do begin                         
 ;    a_bricaud[i] = a_hash[strcompress(string(fix(wl[i])),/remove_all)]
 ;    b_bricaud[i] = b_hash[strcompress(string(fix(wl[i])),/remove_all)]
 ;  endfor                        
 ; original for idl 8 
