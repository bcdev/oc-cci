...
;   
;-
PRO bandShiftCorrectionSetup, correction_context, sensor=sensor
  
  IF (not(keyword_set(sensor))) then sensor='MODISA'
  
  error_status = 11
  
  CASE strupcase(sensor) OF
    'SEAWIFS': BEGIN  
      lambda_i = [412., 490., 510., 555., 555., 555., 667., 667.]
      lambda_o = [413., 488., 531., 531., 547., 560., 665., 670.]
    END
    'MERIS': BEGIN
  ...
  readSpectralDataPureWater,sdata
...
;     
;   number of bandhsift corrected bins, possible zero, in case no inconsistency or incompleteness is detected. Equals the number of 
...
IF intersect_exists THEN BEGIN
  ...
  IF nb_corrected GT 0 THEN BEGIN    
    ...
    Rrs_i = (rrs_matrix[input_wavelength_indexes,*])[*,rrs_valid_index]        
    ...
    rrs_matrix_corrected = correction_factors * RRS_i
  ENDIF
...
PRO weightedAverageEqualCorrectionProducts, rrs_corrected_matrix, rrs_corrected_wavelengths, correction_context
  ...
  IF (n_doubles GT 0) THEN BEGIN
    ...
    foreach double_wl, doubles do begin             
          ...
          rrs_corrected_matrix[wavelength_index[0],*] = total(rebin(rel_weight,n_wl,n_bins) * rrs_corrected_matrix[wavelength_index,*],1)
     endforeach
...
;   Example syntax::
;   
;     bandShiftCorrectionSetup, correction_context, sensor='MODISA' 
...
;                          qaa_bins=qaa_bins, qaa_min=qaa_min, qaa_max=qaa_max, correction_factors=correction_factors
;     save, filename='correction_factor.sav', correction_factors
;     ;write corrected RRS data to file
;       
;   
; :Pre:
;
...
;     
;     Factors can also be calculted by dividing output RRS by input RRS, for those output RRS that were not obtained through a 
...
PRO bandShiftCorrection, rrs_matrix, rrs_wavelengths, rrs_bins, qaa_matrix, correction_context, debug=debug, qaa_bins=qaa_bins, qaa_min=qaa_min, qaa_max=qaa_max, correction_factors=correction_factors
  ...
  IF debug THEN print, 'Corrected ' + strcompress(nb_corrected,/REMOVE_ALL) + ' bins.'
  ...
  rrs_matrix = [rrs_matrix[*,rrs_valid_index],rrs_matrix_corrected]
  rrs_wavelengths = [rrs_wavelengths,rrs_wavelengths_corrected]
  rrs_bins = rrs_bins[rrs_valid_index]      
